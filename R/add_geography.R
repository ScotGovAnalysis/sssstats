...
#'
#' @description
#' This function takes a data frame, and run both `clean_geo_postcode` and
...
#'
#' @details
#' Prior to `sssstats` version 0.2.1 which both `clean_geo_postcode()` and
...
add_geography <- function(input_data,
  ...
  clean_geo_postcode(input_data, !!postcode_column) |>
    add_geo_columns(
      sspl_lookup,
      datazone_lookup,
      simd_lookup
